...
# Local release bundle to install from, instead of resolving the latest GitHub release.
# Typical dev flow:
#   export DUCK_RELEASE_DIR=/path/to/microduck/dist
#   sudo -E sh scripts/install.sh
#
# The directory must contain the signed release payload generated by `cargo xtask package`
# and signed by `cargo xtask sign`, including the bootstrap binary and the manifest.
LOCAL_RELEASE_DIR="${DUCK_RELEASE_DIR:-}"
...
resolve_bootstrap_asset() {
    ...
    if [ -n "$LOCAL_RELEASE_DIR" ]; then
        if [ ! -d "$LOCAL_RELEASE_DIR" ]; then
            die "DUCK_RELEASE_DIR is set but does not exist: ${LOCAL_RELEASE_DIR}"
        fi

        bootstrap_bin="${LOCAL_RELEASE_DIR}/updaterd-bootstrap-aarch64"
        if [ ! -f "$bootstrap_bin" ]; then
            die "DUCK_RELEASE_DIR does not contain the bootstrap binary: ${bootstrap_bin}"
        fi
...
EOF
    ...
    if [ -n "$LOCAL_RELEASE_DIR" ]; then
        if [ ! -f "$BOOTSTRAP_URL" ]; then
            die "local bootstrap binary missing: ${BOOTSTRAP_URL}"
        fi
        cp "$BOOTSTRAP_URL" "${tmp}/updaterd"
    elif ! fetch_asset "$BOOTSTRAP_URL" "${tmp}/updaterd"; then
    ...
    if [ -n "$LOCAL_RELEASE_DIR" ]; then
        GITHUB_TOKEN="$TOKEN" "${tmp}/updaterd" install \
            ...
            --from "$LOCAL_RELEASE_DIR" $force_flag
    else
        GITHUB_TOKEN="$TOKEN" "${tmp}/updaterd" install \
